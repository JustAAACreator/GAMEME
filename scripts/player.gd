...
@onready var rect: ColorRect = $"../Gamemanager/CanvasLayer/playerUI/ColorRect"
@onready var animated_sprite: AnimatedSprite2D = $AnimatedSprite2D
@onready var my_hit_box: Area2D = $MyHitBox
@onready var bar: TextureProgressBar = $"../Gamemanager/CanvasLayer/playerUI/TextureProgressBar"
...
func _physics_process(delta: float) -> void:
	...
	if animationON == false:
		...
		if Input.is_action_just_pressed("dash") and dash_reload <=0 and moving == true and die == false:
			dash_reload = 0.50
			...
			dash = true
			dash_timer = 0.10
		if Input.is_action_just_pressed("kick") and moving == true and die == false:
